...
class DemoRetriever:
        ...
        # kNN检索方法：计算文档之间的相似度，返回前 k 个相似文档
        def retrievekNNDemo(self, permutation, k): 
                documents = []
                with open(os.path.join(self.config.inSet, self.CTI_Source, self.inFile), "r", encoding='utf-8') as f:
                        js = json.load(f)
                        documents.append((js["CTI"], self.inFile))
                for CTI_folder in os.listdir(self.config.demo_set):
                        if CTI_folder.endswith(".json"):
                ...
                ## 写入 CSV 文件
                #documents_df.to_csv('output.csv', index=False, encoding='utf-8')
                #print("DataFrame 已写入 output.csv 文件。")
        ...
        def retriveDemo(self):
                if self.config.retriever["type"] == "kNN":
                        #print("self.config.retriever:",self.config.retriever)
                        #print("self.config.shot:",self.config.shot)
